...
test('furbrief language pills are synced with page language', async ({ page }) => {
  await page.goto('/upload');
  await page.locator('.globe-btn').click();
  await page.locator('.ldrop .lopt', { hasText: 'Español' }).click();

  // The furbrief language pill for Español should be active
  await expect(page.locator('.bpill.on', { hasText: 'Español' })).toBeVisible();
});
